...
Array.cumsum = function(arr) {
    return arr && arr.reduce(function(partial, next) {
        partial.last += next;
        partial.out.push(partial.last);
        return partial;
    }, { out: [], last: 0 }).out;
}
...
Array.reshape = function(base, shape) {
    let indices = Array.cumsum(shape);
    let out = [];
    for (let index = 0; index < indices.length; index++) {
        const start = index == 0 ? 0 : indices[index - 1];
        const end = indices[index];
        out.push(base.slice(start, end));
    }
    return out;
}

Array.takeWhile = function(arr, pred) {
    const out = [];
    for (const x of arr) {
